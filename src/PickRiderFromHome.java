...
public class PickRiderFromHome extends TemplateRideSchedule{

	public PickRiderFromHome(HashMap<String, User> table, User user) {
		super(table, user);
	}
	...
	protected void displayAndPrompt() {
		...
		if((rider != null) && (rider.getMemberSchedule().isAvailableHome()))
		{
			...
			try {
				user.addRideFromHome(rider);
				rider.addRideFromHome(user); // add THIS DRIVER to rider.
